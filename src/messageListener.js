export class MapMessageListener {
  constructor(recordedMessages = null) {
    this.recordedMessages = recordedMessages !== null ? recordedMessages : new Map();
  }
...
export function accumulateMessages(messageListener, messages) {
 ...
 * @param {Object} recordedMessages - map of messages
 * @param {Object} fhirResource - FHIR resource to modify
 * @returns {Object} Modified FHIR resource
 */
export function setCqfMessages(recordedMessages, fhirResource) {
  const cqfMessagesUri = "http://hl7.org/fhir/StructureDefinition/cqf-messages";

  // Ensure the recordedMessages is > 0
  if (recordedMessages.size === 0) {
    return fhirResource;
  }

  const containedResource = toOperationOutcome(recordedMessages);
 ...
 */
const toOperationOutcome = (recordedMessages) => {
  const issueArray = Array.from(recordedMessages.values()).map(toIssue);
...
const toIssue = ({ severity, code, message }) => {
  ...
  return {
    ...
    details: codeToDetails(code),
  };
}

const codeToDetails = (code) => {
